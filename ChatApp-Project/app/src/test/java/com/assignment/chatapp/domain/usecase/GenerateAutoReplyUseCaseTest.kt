...
class GenerateAutoReplyUseCaseTest {
    ...
    private lateinit var useCase: GenerateAutoReplyUseCase

    @Before
    fun setup() {
        repository = mockk(relaxed = true)
        useCase = GenerateAutoReplyUseCase(repository)
    }
    ...
    fun `invoke with valid text returns true and adds user message`() = runTest {
        ...
        coVerify {
            repository.addMessage(
                match { message ->
                    message.text == "Got the message: Hello" &&
    ...
    fun `invoke trims whitespace from message text`() = runTest {
        ...
        coVerify {
            repository.addMessage(
                match { it.text == "Got the message: Hello" }
            )
        }
    }
